...
public class WirelessReceiverScreen extends AbstractContainerScreen<WirelessReceiverMenu> {
    ...
    public void render(GuiGraphics gfx, int mouseX, int mouseY, float partialTick) {
        ...
        if (channelBox != null && channelBox.isHoveredOrFocused()) {
            gfx.renderTooltip(font,
                    List.of(
                            Component.translatable("gui.swr.receiver.channel_box.tooltip.line1").withStyle(ChatFormatting.GOLD).getVisualOrderText(),
                            Component.translatable("gui.swr.receiver.channel_box.tooltip.line2").withStyle(ChatFormatting.ITALIC).getVisualOrderText()
                    ),
                    mouseX, mouseY);
        }
    }
